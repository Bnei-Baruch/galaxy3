...
it("messaged delivery guaranteed", (done) => {
  const gdmA = new GuaranteeDeliveryManager(
    "userA",
    /* intervalsDelay =*/ 100,
    new SendOptions(/* maxDelay= */ 2000, /* retryDelay= */ 500)
  );
  const userA = new User(/* messageSendDelay= */ 10);
  const gdmB = new GuaranteeDeliveryManager(
    "userB",
    /* intervalsDelay =*/ 100,
    new SendOptions(/* maxDelay= */ 2000, /* retryDelay= */ 500)
  );
  ...
  userB.setOnMessageCallback((message) => {
    gdmB
      .accept(message, (msg) => userA.send(Object.assign({}, msg)))
      .then((message) => {
        if (message === null) {
  ...
  gdmA
    .send(Object.assign({}, theMessage), /* toAck= */ [], (msg) => userB.send(Object.assign({}, msg)))
    .then(() => {
...
it("slow user, retry few times", (done) => {
  const gdmA = new GuaranteeDeliveryManager(
    "userA",
    /* intervalsDelay =*/ 10,
    new SendOptions(/* maxDelay= */ 200, /* retryDelay= */ 50)
  );
  ...
  gdmA
    .send(Object.assign({}, theMessage), /* toAck= */ [], (msg) => {
      retries++;
      userB.send(Object.assign({}, msg));
    })
    .then(() => {
...
it("very slow user, custom long send delay", (done) => {
  ...
  gdmA
    .send(
      Object.assign({}, theMessage),
      /* toAck= */ [],
      (msg) => {
        retries++;
        userB.send(Object.assign({}, msg));
      },
      new SendOptions(/* maxDelay= */ 500, /* retryDelay= */ 100)
    )
    .then(() => {
      console.log("very slow user, fail eventually - UserA success", theMessage);
      expect(retries).toBe(3);
...
it("can explicitl ack myself", (done) => {
  ...
  gdm
    .send(Object.assign({}, theMessage), /* toAck= */ ["userA"], (msg) => userB.send(Object.assign({}, msg)))
    .then(() => {
...
const createRoom = (numUsers) => {
  ...
  for (let i = 0; i < numUsers; ++i) {
    ...
    room[userId] = {
      gdm: new GuaranteeDeliveryManager(
        userId,
        /* intervalsDelay =*/ 100,
        new SendOptions(/* maxDelay= */ 2000, /* retryDelay= */ 500)
      ),
    ...
    room[userId].user.setOnMessageCallback((message) => {
      ...
      room[userId].gdm
        .accept(message, sendToAll(room, userId))
        .then((message) => {
          if (message === null) {
...
it("messaged delivery guaranteed to many users any", (done) => {
  ...
  room["user_0"].gdm
    .send(Object.assign({}, theMessage), /* toAck= */ [], sendToAll(room, "user_0"))
    .then(() => {
...
it("messaged delivery guaranteed to many users - specific user ack", (done) => {
  ...
  const theMessage = {test: "test"};
  room["user_0"].gdm
    .send(Object.assign({}, theMessage), /* toAck= */ ["user_5"], sendToAll(room, "user_0"))
    .then(() => {
...
it("messaged delivery guaranteed to many users - some users ack", (done) => {
  ...
  const theMessage = {test: "test"};
  room["user_0"].gdm
    .send(Object.assign({}, theMessage), /* toAck= */ ["user_5", "user_2", "user_9"], sendToAll(room, "user_0"))
    .then(() => {
...
it("messaged delivery guaranteed to many users - some users ack - one of them unreachable", (done) => {
  ...
  room["user_0"].gdm
    .send(
      Object.assign({}, theMessage),
      /* toAck= */ ["user_5", "user_2", "user_9"],
      sendToAll(room, "user_0", /* unreachableUsers= */ ["user_2"])
    )
    .then(() => {
    ...
    .catch((error) => {
      console.log(
        "messaged delivery guaranteed to many users - some users ack - one of them unreachable - failed",
        error
      );
      expect(error.reason).toBe(DEADLINE_EXCEEDED);
      done();
