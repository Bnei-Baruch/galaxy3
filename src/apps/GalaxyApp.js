    ...
    return roles
      .map((role, i) => {
        if (role.match(/^(new_user|gxy_guest|gxy_pending_approval|gxy_user|pending_approval)$/)) {
          return (
            <Button key={i} size="massive" color="green" onClick={() => window.open("user", "_self")}>
              Arvut
            </Button>
          );
        }
        if (role === "gxy_shidur") {
          return (
            <Button key={i} size="massive" color="green" onClick={() => window.open("shidur", "_self")}>
              Shidur
            </Button>
          );
        }
        if (role === "gxy_sndman") {
          return (
            <Button key={i} size="massive" color="green" onClick={() => window.open("sndman", "_self")}>
              SoundMan
            </Button>
          );
        }
        if (role.match(/^(gxy_admin|gxy_root|gxy_viewer)$/)) {
          return (
            <Button key={i} size="massive" color="green" onClick={() => window.open("admin", "_self")}>
              Admin
            </Button>
          );
        }
