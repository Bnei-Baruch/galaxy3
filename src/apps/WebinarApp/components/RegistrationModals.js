...
export const RegistrationModals = ({user, language, updateUserRole}) => {
  ...
  if (role !== userRolesEnum.pending_approve && role !== userRolesEnum.new_user) return null;
  ...
  const renderCompleted = () => {
    return (
      <Dialog open={modalState === modalStateEnum.completed} onClose={handleClose} dir={direction}>
        <DialogContent>
  ...
  const renderGoToComplete = () => (
    <Typography className={classes.linkContainer} onClick={() => setModalState(modalStateEnum.form)} dir={direction}>
