  ...
  return (
    <div className="mediaplayer__volume">
      <button type="button" onClick={handleMuteUnmute}>
        {(muted || volumeState === 0) && <Icon key="mute" name="volume off" color="red" />}
        {!muted && volumeState > 0 && volumeState < 0.5 && <Icon key="volume-down" name="volume down" color="white" />}
        {!muted && volumeState >= 0.5 && <Icon key="volume-up" name="volume up" color="white" />}
      </button>
    </div>
  );
};
