...
class MobileStreaming extends Component {
    ...
    checkData = (json) => {
        ...
        if(localStorage.getItem("mob_extip") === ip)
    ...
    streamGalaxy = (talk,col,name) => {
        if(talk) {
            ...
            if(name.match(/^(newyork|toronto|chicago)$/)) {
                ...
                this.initTranslationStream(trllang[localStorage.getItem("mob_langtext")] || 303);
            }
            ...
            let abody = { "request": "switch", "id": Number(localStorage.getItem("mob_lang")) || 15};
    ...
    setVideo = (videos) => {
        ...
        localStorage.setItem("mob_video", videos);
    };
    ...
    setAudio = (audios,options) => {
        ...
        localStorage.setItem("mob_lang", audios);
        localStorage.setItem("mob_langtext", text);
    };
