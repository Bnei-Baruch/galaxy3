...
import {ListItemIcon, ListItemText, Divider, Typography, TextField, ListItem} from "@mui/material";
...
export const SelectLanguage = ({setAudio}) => {
  ...
  const renderHeader = (op) => (
    <ListItem key={op.text}>
      ...
      <ListItemText primary={t(op.text)} secondary={t(op.description)} />
    </ListItem>
  );
  ...
  return (
    <TextField
      ...
      onChange={({
        target: {
          value: {value, eng_text},
        },
      }) => handleSelectLang(value, eng_text)}
    >
